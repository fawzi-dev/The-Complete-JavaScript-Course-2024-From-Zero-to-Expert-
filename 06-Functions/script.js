...
choose, and an array with the number of replies for each option. 
...
const poll = {
  ...
  registerNewAnswer: function () {
    ...
    // Answers 
    const formattedAnswer = this.options.map((option, index) => {
      const [key, value] = option.split(': ');

      return `${key}: ${value}`;
    }).join('\n');

    const userOption = prompt(`What is your favourite programming language?\n${formattedAnswer}\n(Write option number)`);
...
const newPollResutl = poll.registerNewAnswer();
